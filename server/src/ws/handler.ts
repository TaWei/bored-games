...
export async function handleWebSocket(
  ...
  // Get or create game loop (pass redisSub to avoid circular require)
  const loop = getOrCreateGameLoop(roomCode, redisSub as any);
  ...
  ws.on('message', (data: string | Buffer) => {
    ...
    loop.handleMessage(sessionId, raw);
  });

  ws.on('close', async () => {
    loop.removeConnection(sessionId);
    ...
    if (loop.connectionCount === 0) {
      ...
      }, 30_000); // Give 30s for reconnections
    }
  });

  ws.on('error', (err: Error) => {
    console.error(`WS error for session ${sessionId} in room ${roomCode}:`, err.message);
  });
}
